...
class TestClassLoader(TestCase):
    ...
    def test_import_local(self):
        with mock.patch.object(test_module.sys, "modules", {}):
            assert (
                ClassLoader.load_module("aries_cloudagent_vsw.transport").__name__
                == "aries_cloudagent_vsw.transport"
            )

    def test_import_relative(self):
        with mock.patch.object(test_module.sys, "modules", {}):
            assert (
                ClassLoader.load_module("transport", "aries_cloudagent_vsw").__name__
                == "aries_cloudagent_vsw.transport"
            )
        with mock.patch.object(test_module.sys, "modules", {}):
            assert (
        ...
        with mock.patch.object(test_module.sys, "modules", {}):
            assert (
                ClassLoader.load_module(
                    "..transport", "aries_cloudagent_vsw.config"
                ).__name__
                == "aries_cloudagent_vsw.transport"
            )
    ...
    def test_import_missing(self):
        with mock.patch.object(test_module.sys, "modules", {}):
            assert ClassLoader.load_module("aries_cloudagent_vsw.not") is None
        with mock.patch.object(test_module.sys, "modules", {}):
            assert ClassLoader.load_module("aries_cloudagent_vsw.not.a-module") is None
        with mock.patch.object(test_module.sys, "modules", {}):
            assert ClassLoader.load_module("aries_cloudagent_vsw", "not.a-module") is None
    ...
    def test_import_error(self):
        with mock.patch.object(
            ...
            with self.assertRaises(ModuleLoadError):
                ClassLoader.load_module("aries_cloudagent_vsw.config")
    ...
    def test_load_class_missing(self):
        ...
        with self.assertRaises(ClassNotFoundError):
            assert ClassLoader.load_class("aries_cloudagent_vsw.NotAClass")
        with self.assertRaises(ClassNotFoundError):
            assert ClassLoader.load_class("not-a-module.NotAClass")
        with self.assertRaises(ClassNotFoundError):
            # should be a string, not a type
            assert ClassLoader.load_class("aries_cloudagent_vsw.version.__version__")

    def test_load_subclass(self):
        assert ClassLoader.load_subclass_of(BaseError, "aries_cloudagent_vsw.config.base")

    def test_load_subclass_missing(self):
        with self.assertRaises(ClassNotFoundError):
            assert ClassLoader.load_subclass_of(
                TestCase, "aries_cloudagent_vsw.config.base"
            )
        with self.assertRaises(ClassNotFoundError):
            assert ClassLoader.load_subclass_of(
                TestCase, "aries_cloudagent_vsw.not-a-module"
            )

    def test_scan_packages(self):
        pkgs = ClassLoader.scan_subpackages("aries_cloudagent_vsw")
        assert "aries_cloudagent_vsw.transport" in pkgs
        pkgs = ClassLoader.scan_subpackages("aries_cloudagent_vsw.transport")
        assert "aries_cloudagent_vsw.transport.inbound" in pkgs

    def test_scan_packages_missing(self):
        with self.assertRaises(ModuleLoadError):
            ClassLoader.scan_subpackages("aries_cloudagent_vsw.not-a-module")
